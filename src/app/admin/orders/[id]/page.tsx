...
const AdminOrderDetailContent = memo(function AdminOrderDetailContent() {
  ...
  const { documents, loading: documentsLoading, refetch: refetchDocs } =
    useAdminOrderDocuments(orderId);

  const { adminUpdateOrderStatus, loading: statusLoading } =
    useAdminUpdateOrderStatus();
  const { adminDeleteOrder, loading: deleteLoading } = useAdminDeleteOrder();
  ...
  const handleStatusChange = useCallback(
    async (status: OrderStatus) => {
      ...
      try {
        ...
        showToast("error", (e as Error)?.message ?? "Ошибка смены статуса");
      }
    },
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      showToast("error", (e as Error)?.message ?? "Ошибка удаления");
    } finally {
  ...
  const handleGenerateContract = useCallback(async () => {
    try {
      ...
      showToast("error", (e as Error)?.message ?? "Ошибка формирования договора");
    }
  ...
  if (loading && !order) {
    return (
      <Surface variant="floating" className="animate-pulse rounded-2xl p-8">
  ...
  if (!order) {
    return (
      <Surface variant="floating" className="rounded-2xl p-8 text-center">
  ...
  const displayNumber = (order as OrderFieldsFragment & { number?: string | null }).number ?? order.id;
                  ...
                  >
                    <FileText
                      className={cn(
                        ...
                        isDocx && "text-blue-600 dark:text-blue-400",
